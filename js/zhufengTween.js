~function () {
   /* var effect = { //这一组对象里都是存放如何去计算当前位置的公式
     ...
     * @param target --object数据类型-需要运动的终点
     * @param duration --需要运动的时间间隔
     * @param effect --运动的类型（匀速/渐快/渐慢...）
     * @param callback --move函数执行结束之后需要继续调用的函数并执行
     */
    function move(curEle, target, duration,effect,callback) { //驱动元素运动的 //target{width:100,left:50}
        ...
        //接下来我们需要去开启定时器并且完成动画
        clearInterval(curEle.timer);//重中之重！！！！
...
duration); //获取单个维度的值
                    utils.setCss(curEle,k,curPosi/*维度*//*维度的值*/);
                }
            }
        },interval);
    }
